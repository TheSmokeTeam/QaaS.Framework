...
public class ByNameObjectCreator(ILogger logger) : IByNameObjectCreator
{
    ...
    )
    {
        ...
        logger.LogDebug(
            "Found subclass of type {TType}, the class's full name is"
                + " {SubClassFullName}, Initializing it.",
            typeof(T).Name,
            subClassType.FullName
        );
        return (T)
    ...
    private Type ResolveSubClassType<T>(
        string classManifestationName,
        IEnumerable<Assembly>? assemblies
    )
    {
        ...
        var fullNameMatches = candidateTypes
            ...
            .Where(type =>
                string.Equals(type.FullName, classManifestationName, StringComparison.Ordinal)
                || string.Equals(
                    type.AssemblyQualifiedName,
                    classManifestationName,
        ...
        if (fullNameMatches.Count > 1)
            throw new AmbiguousMatchException(
                CreateAmbiguousTypeMessage(typeof(T), classManifestationName, fullNameMatches)
            );

        foreach (var candidateTypesInAssembly in candidateTypes)
        {
            var simpleNameMatches = candidateTypesInAssembly
                .Types.Where(type =>
                    string.Equals(type.Name, classManifestationName, StringComparison.Ordinal)
                )
                .Distinct()
                .ToList();
    ...
    private static string CreateAmbiguousTypeMessage(
        Type requestedBaseType,
        string classManifestationName,
    ...
    )> GetCandidateTypesByAssembly<T>(IEnumerable<Assembly>? assemblies)
    {
