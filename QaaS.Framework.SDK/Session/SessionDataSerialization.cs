...
public static class SessionDataSerialization
{
    ...
    /// </summary>
    public static SessionData DeserializeSessionData(
        byte[] serializedData,
        JsonSerializerOptions? options = null
    )
    {
        var serializedSessionData =
            JsonSerializer.Deserialize<SerializedSessionData>(serializedData, options)
            ?? throw new ArgumentException(
                "Received null when attempting to deserialize session data"
            );
    ...
    )
    {
        var deserializer = DeserializerFactory.BuildDeserializer(
            serializedCommunicationData.SerializationType
        );
        ...
        if (deserializer == null)
            ...
            {
                Data = serializedCommunicationData
                    .Data.Select(item => new DetailedData<object>
                    {
    ...
    /// serialization configurations
    /// </summary>
    public static byte[] SerializeSessionData(
        SessionData sessionData,
        JsonSerializerOptions? options = null
    )
    {
        return JsonSerializer.SerializeToUtf8Bytes(
            new SerializedSessionData
            {
    ...
    /// </summary>
    public static SerializedCommunicationData SerializeCommunicationData(
        CommunicationData<object> communicationData
    )
    {
        ...
        if (serializer == null)
            return serializedCommunicationData with
            {
                Data = communicationData
                    .Data.Select(item => new SerializedDetailedData
                    {
                        MetaData = item.MetaData,
                        Timestamp = item.Timestamp,
                        Body = (byte[]?)item.Body,
