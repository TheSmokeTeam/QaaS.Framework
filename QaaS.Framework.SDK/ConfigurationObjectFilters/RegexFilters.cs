...
public static class RegexFilters
{
    ...
    public static bool DataSource(DataSource data, string pattern) =>
        Regex.IsMatch(data.Name, pattern);
    ...
    /// <param name="pattern"> the Regex pattern to filter by </param>
    /// <returns> True if passed the filter, False otherwise. </returns>
    public static bool SessionData(SessionData data, string pattern) =>
        Regex.IsMatch(data.Name, pattern);
}
