...
public class ProtocolAdvancedBehaviorTests
{
    ...
    public void S3Protocol_ReadChunk_PreservesBytesForMultipleDeserializers()
    {
        ...
        var scenarios = new[]
        {
            ...
            new S3DeserializationScenario(
                "binary-replaced-with-json",
                // BinaryFormatter is disabled for security (F-2 fix). This scenario now uses
                // Json serialization to preserve the multi-deserializer coverage intent.
                new QaaS.Framework.Serialization.Serializers.Json().Serialize(binaryPayload)!,
                bytes =>
                    new QaaS.Framework.Serialization.Deserializers.Json().Deserialize(
                        bytes,
                        typeof(SerializableS3Payload)
