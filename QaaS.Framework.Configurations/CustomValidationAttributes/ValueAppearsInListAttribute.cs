...
public class ValueAppearsInListAttribute : ValidationAttribute
{
    ...
    {
        _listPropertyName =
            listPropertyName ?? throw new ArgumentNullException(nameof(listPropertyName));
        _propertyInListName =
    ...
    protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
    {
        ...
        var listValues = (
            ...
            select propertyInfo.GetValue(item)
        )
            .OfType<object>()
            .ToList();
        ...
        foreach (var item in enumerable)
        {
            ...
            var validValues = string.Join(
                ", ",
                listValues.Distinct().Select(v => v.ToString() ?? "null")
            );
