...
/// </summary>
public class AllItemsInEnumerablePropertyInEnumerableExistAsPropertyInEnumerableAttribute
    : ValidationAttribute
{
    ...
    public AllItemsInEnumerablePropertyInEnumerableExistAsPropertyInEnumerableAttribute(
        string enumerablePropertyName,
        string propertyName
    )
    {
    ...
    protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
    {
        ...
        var propertyValues = (
            ...
            select (
                item.GetType().GetProperty(_propertyName)
                ?? throw new ArgumentException($"Could not find {_propertyName} property")
            ).GetValue(item)
                ?? throw new ArgumentException(
                    $"Could not find {_propertyName} property in one of the enumerable items"
                )
        ).ToList();
        foreach (var item in enumerable)
        {
            var enumerableProperty =
                ...
                ?? throw new ArgumentException(
                    $"Could not find {_enumerablePropertyName} property"
                );

            if (enumerableProperty.GetValue(item) is not IEnumerable enumerablePropertyValue)
                throw new ArgumentException(
                    $"{_enumerablePropertyName} property is not an enumerable"
                );

            if (
                enumerablePropertyValue
                    .Cast<object?>()
                    .Any(enumerablePropertyItem => !propertyValues.Contains(enumerablePropertyItem))
            )
                return new ValidationResult(
                    $"{_enumerablePropertyName} contains an item not"
