...
public class EnumerablePropertyDoesNotContainAnotherPropertyValueAttribute : ValidationAttribute
{
    ...
    public EnumerablePropertyDoesNotContainAnotherPropertyValueAttribute(string propertyName)
    {
    ...
    protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
    {
        ...
        var objectPropertyValue = (
            validationContext.ObjectType.GetProperty(_propertyName)
        ...
        if (enumerable.Cast<object?>().Contains(objectPropertyValue))
        {
            return new ValidationResult(
                $"{validationContext.MemberName} contains an item with the same value as"
