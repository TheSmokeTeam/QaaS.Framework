...
public static class ConfigurationCollapseParser
{
    ...
    public static IConfiguration CollapseShiftLeftArrowsInConfiguration(
    ...
    > GetConfigurationPathsAndValuesWithCollapsedArrows(IConfiguration configurationRoot)
    {
        ...
        return GetDirectChildrenAfterCollapsingArrows(configurationRoot)
            .Aggregate(
                ...
                (current, configurationSection) =>
                    current
                        .Concat(
                            GetConfigurationPathsAndValuesWithCollapsedArrows(configurationSection)
                        )
                        .ToList()
            );
    }
    ...
    private static IEnumerable<
        KeyValuePair<string, string?>
    > GetConfigurationPathsAndValuesWithCollapsedArrows(
    ...
    private static IEnumerable<
        KeyValuePair<IConfigurationSection, int>
    > GetDirectChildrenAndNumberOfCollapsedArrows(
        IConfiguration configuration,
        int numberOfArrows = 0
    )
    {
        var directChildren = configuration.GetChildren().ToList();
        var directChildrenAfterCollapsingArrows =
            new List<KeyValuePair<IConfigurationSection, int>>();
        foreach (var child in directChildren)
        {
            ...
            if (!child.GetChildren().Any())
            {
                throw new InvalidConfigurationsException(
                    $"The collapse key '{ConfigurationConstants.CollapseString}' at "
