...
public static class DateTimeExtensions
{
    private static readonly string IsraelTimeZoneId = Environment.OSVersion.Platform == PlatformID.Unix
        ? Path.Join("Asia", "Jerusalem") : "Israel Standard Time";
    ...
    /// (for example in israel its 3 for gmt + 3 during summer)</param>
    /// <param name="isDayLightSavingTime"> True if its day light saving time right now in israel time zone,
    ...
    private static bool IsDayLightSavingTimeInGivenDateTime(this DateTime dateTime) =>
        TimeZoneInfo.FindSystemTimeZoneById(IsraelTimeZoneId).IsDaylightSavingTime(dateTime);
}