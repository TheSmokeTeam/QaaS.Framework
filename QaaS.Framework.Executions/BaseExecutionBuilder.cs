...
public abstract class BaseExecutionBuilder<TContext, TExecutionData>
    where TContext : BaseContext<TExecutionData>
    where TExecutionData : class, IExecutionData, new()
{
    [
        ...
        Description(
            "List of data sources that can be used in the rest of the execution."
                + " They provide data that can be sent to the tested system or used by the execution itself to perform "
                + "a multitude of logics."
        )
    ]
    public DataSourceBuilder[]? DataSources { get; internal set; } = [];
