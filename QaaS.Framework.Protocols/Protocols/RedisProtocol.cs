...
public class RedisProtocol(RedisSenderConfig configuration, ILogger logger) : IChunkSender
{
    ...
    public IEnumerable<DetailedData<object>> SendChunk(IEnumerable<Data<object>> chunkDataToSend)
    {
        ...
        for (var retryCount = 0; retryCount < configuration.Retries; retryCount++)
        {
            // A Redis ITransaction is single-shot: rebuild it on every attempt.
            ...
            catch (Exception redisException)
            {
                ...
                // retryCount is incremented by the for-loop step only — no extra increment here.
            }
        }

        throw new RedisException($"Failed to send to redis after {configuration.Retries} retries");
    }
