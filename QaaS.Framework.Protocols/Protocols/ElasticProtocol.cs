...
public class ElasticProtocol : IChunkReader, IChunkSender, IDisposable
{
    ...
    /// </summary>
    public ElasticProtocol(ElasticReaderConfig configuration, DataFilter dataFilter, ILogger logger)
        : this((ElasticIndicesRegex)configuration, dataFilter, logger)
    {
    ...
    /// </summary>
    public ElasticProtocol(ElasticIndicesRegex configuration, DataFilter dataFilter, ILogger logger)
        : this(configuration, configuration.IndexPattern!, dataFilter, logger)
    {
    ...
    /// </summary>
    public ElasticProtocol(ElasticSenderConfig configuration, DataFilter dataFilter, ILogger logger)
        : this(configuration, configuration.IndexName!, dataFilter, logger)
    {
    ...
    /// </summary>
    public ElasticProtocol(
        BaseElasticConfig configuration,
        string indexName,
    ...
    public IEnumerable<DetailedData<object>> ReadChunk(TimeSpan timeout)
    {
        ...
        {
            var descriptor = s.Scroll(
                    new Time(
                        TimeSpan.FromMilliseconds(_readerConfiguration.ScrollContextExpirationMs)
                    )
                )
                .Size(_readerConfiguration.ReadBatchSize)
                // Query items according to user match query
                .Query(q => q.QueryString(qs => qs.Query(_readerConfiguration.MatchQueryString)))
                // Arrange items from first to last
                .Sort(sort => sort.Ascending(_readerConfiguration.TimestampField));
                ...
                descriptor = descriptor.Query(q =>
                    q.Bool(b =>
                        b.Must(
                            mu =>
                                mu.DateRange(r =>
                                    r.Field(_readerConfiguration.TimestampField)
                                        .GreaterThan(utcConsumptionStartTime)
                                ),
                            mu =>
                                mu.QueryString(qs =>
                                    qs.Query(_readerConfiguration.MatchQueryString)
                                )
                        )
                    )
                );
            ...
            if (!_dataFilter.Body)
                descriptor = descriptor.Source(src =>
                    src.Includes(i => i.Field(_readerConfiguration.TimestampField))
                );
            return descriptor;
        };
    ...
    public IEnumerable<DetailedData<object>> SendChunk(IEnumerable<Data<object>> chunkDataToSend)
    {
        ...
        if (bulkResponse.Errors || !bulkResponse.ApiCall.Success)
            throw new ElasticsearchClientException(
                $"Failed to publish documents batch to elastic index"
    ...
    private IEnumerable<DetailedData<object>> ScrollRead(
        Func<ISearchResponse<dynamic?>> scrollRequest
    )
    {
        ...
        while (scrollResponse.Documents.Any())
        {
            foreach (var document in scrollResponse.Documents)
            {
                yield return new DetailedData<object>
                {
                    ...
                    MetaData = null,
                    Timestamp =
                        GetUtcTimeFromDocument(document)
                        ?? throw new TargetException(
                            "Failed to get timestamp of queried data that matches "
    ...
    {
        var searchResponse = _elasticClient.Search<dynamic?>(s =>
            s
            // Query items according to user match query
            .Query(q => q.QueryString(qs => qs.Query(_readerConfiguration.MatchQueryString)))
                // Only take the timestamp field from the queried items
                .Source(src => src.Includes(i => i.Field(_readerConfiguration.TimestampField)))
                // Only take the item with the latest timestamp field
                .Sort(sort => sort.Descending(_readerConfiguration.TimestampField))
                .Size(1)
        );
        ...
        if (searchResponse.IsValid && latestTime is not null)
        {
            if (latestTime.Value.Kind != DateTimeKind.Unspecified)
                return (long)
                    Math.Round(
                        (
                            GetCurrentDateTimeUtc() - latestTime.Value.ToUniversalTime()
                        ).TotalMilliseconds,
            ...
            _logger.LogCritical(
                "Latest insertion to index pattern {IndexPattern} that matches "
                ...
                _readerConfiguration.MatchQueryString
            );
            return null;
        }

        _logger.LogInformation(
            "Latest insertion to index pattern {IndexPattern} that matches {ElasticMatchQuery}"
    ...
    private void WaitUntilConsumptionTimeoutIsReached(TimeSpan timeout)
    {
        ...
        {
            Thread.Sleep(
                (int)(timeout.TotalMilliseconds - milliSecondsSinceLastElasticIndexInsertion)
            );
            ...
            _logger.LogDebug(
                "Since last insertion to elastic index pattern {IndexPattern} "
    ...
    private DateTime? GetUtcTimeFromDocument(dynamic? document)
    {
        if (document == null)
            return null;
        var pathResults = (
            (JsonDocument?)JsonSerializer.SerializeToDocument(document)
    ...
    public void EmptyElasticIndices()
    {
        ...
        Parallel.ForEach(
            ...
            index =>
            {
                var response = _elasticClient.DeleteByQuery<object>(d =>
                    d.Index(index)
                        .Query(q =>
                            q.QueryString(qs => qs.Query(_readerConfiguration.MatchQueryString))
                        )
                        .Conflicts(Conflicts.Proceed)
                );
                if (response is not { IsValid: true } || response.ApiCall is not { Success: true })
                    throw new Exception($"Failed to empty index {index} because {response}");

                _logger.LogDebug("Index {EmptiedIndex} emptied successfully", index);
            }
        );
        _logger.LogInformation(
            "All indexes under index pattern {EmptiedIndexPattern} emptied successfully",
            _readerConfiguration.IndexPattern!
        );
    }
    ...
    protected virtual IEnumerable<string> GetIndexNames(string indexPattern)
    {
        return _elasticClient
            .Cat.Indices(new CatIndicesRequest(indexPattern))
            .Records.Select(record => record.Index);
    }
