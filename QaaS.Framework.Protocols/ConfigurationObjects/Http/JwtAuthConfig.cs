...
public record JwtAuthConfig
{
    ...
    [
        ...
        Description(
            $"Custom Hierarchical claims of the JWT, Must be a string in yaml format. "
    ...
    [
        Description("The JWT Algorithm algorithm used"),
        DefaultValue(JwtAlgorithms.HMACSHA256Algorithm)
    ]
    ...
    [
        Description(
            $"Whether to build JWT config with claims or send the {nameof(Secret)} value as the auth token"
        ),
        DefaultValue(true)
    ]
