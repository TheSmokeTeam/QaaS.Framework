...
public record KafkaTopicSenderConfig : BaseKafkaTopicProtocolConfig, ISenderConfig
{
    ...
    [
        Description(
            "Time interval in milliseconds to wait between each retry of Kafka Topic message send."
        ),
    ...
    [
        Description("Maximum total message size sum allowed on the inner producer queue."),
    ...
    [
        Description("Compression type to use before sending messages"),
    ...
    // Bounds mirror librdkafka's `message.max.bytes` valid range (1000 .. 1000000000) as published in the
    // Confluent.Kafka CONFIGURATION.md; values outside this range are rejected by the producer at construction.
    [Range(1_000, 1_000_000_000)]
    [Description(
        "Maximum allowed Kafka message size in bytes (librdkafka `message.max.bytes`). "
            + "Valid range is 1000 to 1000000000; must not exceed broker/topic limits."
    )]
