...
public record KafkaTopicReaderConfig : BaseKafkaTopicProtocolConfig, IReaderConfig
{
    ...
    [
        Description(
            "Where the reader starts reading from in the topic when being created, latest means it starts "
    ...
    [
        Description(
            "Whether the read mechanism will commit the offsets automatically and periodically in the background."
        ),
        DefaultValue(true)
    ]
    ...
    [
        Description(
            "The name of a partition assignment strategy to use. The elected group leader will use a strategy "
    ...
    [
        Range(0, int.MaxValue),
        Description(
            "Maximum allowed time between calls to read messages for high-level"
    ...
    [
        Range(0, int.MaxValue),
        Description(
            "Minimum number of bytes the broker responds with. If `FetchWaitMaxMs`"
    ...
    [Description(
        "Maximum allowed Kafka message size in bytes. Must not exceed broker/topic limits."
    )]
