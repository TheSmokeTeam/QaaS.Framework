...
public record S3BucketSenderConfig : S3BucketConfig, ISenderConfig
{
    ...
    public ObjectNamingGeneratorType S3SentObjectsNaming { get; set; } =
        ObjectNamingGeneratorType.GrowingNumericalSeries;

    [
        Description(
            "The number of times to retry sending to S3 in case the s3 maximum IO is reached"
        ),
        DefaultValue(null)
    ]
    ...
    [
        Description(
            $"S3 Storage Class Definitions. Options:"
    ...
    public S3StorageClassEnum S3StorageClass { get; set; } =
        S3StorageClassEnum.StandardInfrequentAccess;
}
