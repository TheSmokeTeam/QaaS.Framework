...
public record RabbitMqSenderConfig : BaseRabbitMqConfig, ISenderConfig
{
    ...
    [Description("Default AppId to send messages with, if the message" +
                 " doesn't contain AppId in its MetaData this one is Used"), DefaultValue(null)]
    public string? AppId { get; set; } = null;

    [Description("Default ClusterId to send messages with, if the message" +
                 " doesn't contain ClusterId in its MetaData this one is Used"), DefaultValue(null)]
    public string? ClusterId { get; set; } = null;

    [Description("Default ContentEncoding to send messages with, if the message" +
                 " doesn't contain ContentEncoding in its MetaData this one is Used"), DefaultValue(null)]
    ...
    [Description("Default CorrelationId to send messages with, if the message" +
                 " doesn't contain CorrelationId in its MetaData this one is Used"), DefaultValue(null)]
    public string? CorrelationId { get; set; } = null;

    [Range(1, 2), Description("Default DeliveryMode to send messages with. Valid values are 1 (transient) and 2 (persistent). " +
                              "If the message doesn't contain DeliveryMode in its MetaData this one is Used"),
     DefaultValue(null)]
    public int? DeliveryMode { get; set; } = null;

    [Description("Default MessageId to send messages with, if the message" +
                 " doesn't contain MessageId in its MetaData this one is Used"), DefaultValue(null)]
    public string? MessageId { get; set; } = null;

    [Description("Default Persistent flag to send messages with, if the message" +
                 " doesn't contain Persistent or DeliveryMode in its MetaData this one is Used. " +
                 $"{nameof(DeliveryMode)} takes precedence when both are configured."), DefaultValue(null)]
    public bool? Persistent { get; set; } = null;

    [Range(0, 255), Description("Default Priority to send messages with, if the message" +
                                " doesn't contain Priority in its MetaData this one is Used"), DefaultValue(null)]
    public int? Priority { get; set; } = null;

    [Description("Default ReplyTo to send messages with, if the message" +
                 " doesn't contain ReplyTo in its MetaData this one is Used"), DefaultValue(null)]
    public string? ReplyTo { get; set; } = null;

    [Range(typeof(long), "0", "9223372036854775807"),
     Description("Default RabbitMQ timestamp as Unix time seconds, if the message" +
                 " doesn't contain TimestampUnixTime in its MetaData this one is Used"), DefaultValue(null)]
    ...
    [Description("Default UserId to send messages with, if the message" +
                 " doesn't contain UserId in its MetaData this one is Used"), DefaultValue(null)]
