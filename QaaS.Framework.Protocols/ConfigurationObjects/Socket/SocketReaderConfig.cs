...
public record SocketReaderConfig : BaseSocketConfig, IReaderConfig
{
    [
        ...
        DefaultValue(5000),
        Description("Timeout receiving a packet in milliseconds")
    ]
    public int ReceiveTimeoutMs { get; set; } = 5000;

    [
        Range(0, int.MaxValue),
        DefaultValue(1024 * 2 * 2 * 2 * 2 * 2 * 2),
    ...
    [
        DefaultValue(null),
        Description(
            "Character to seperate read buffers into messages (delimiter) - when left"
