...
public record SocketSenderConfig : BaseSocketConfig, ISenderConfig
{
    [
        ...
        DefaultValue(5000),
        Description("socket's timeout sending a packet in milliseconds")
    ]
    ...
    [
        NagleAlgorithmIsFalseWhenProtocolTypeIsNotTcp,
        Description(
            "Whether to use the Nagle Algorithm (true) or not(false)."
                ...
                + " but it can sometimes cause delays in the transmission of large packets."
        ),
        DefaultValue(false)
    ]
    ...
    [
        ...
        Description(
            "the number of seconds to remain connected after sending all the data,"
    ...
    protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
    {
        ...
        return config.NagleAlgorithm && config.ProtocolType != ProtocolType.Tcp
            ? new ValidationResult(
                $"{nameof(config.NagleAlgorithm)} can only be enabled when"
