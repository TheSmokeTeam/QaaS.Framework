...
public record ElasticReaderConfig : ElasticIndicesRegex, IReaderConfig
{
    [
        Description(
            "The json path to the main timestamp field to use for deciding a queried item's latest update time"
        ),
    ...
    [
        Description(
            "Whether to only read messages that arrived to the elastic after the start of the read action"
                + " (true) or read all messages regardless of arrival time (false)"
        ),
        DefaultValue(false)
    ]
    ...
    [
        ...
        Description(
            $"If the {nameof(ReadFromRunStartTime)} is enabled, this property specifies how "
                + $"far before the start of the read action to start consuming messages in seconds"
        ),
        DefaultValue(0)
    ]
