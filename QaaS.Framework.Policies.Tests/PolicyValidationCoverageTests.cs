...
public class PolicyValidationCoverageTests
{
    ...
    public void AdvancedLoadBalancePolicyConfig_RequiresAmountOrTimeoutPerStage()
    {
        ...
        var invalid = Validator.TryValidateObject(
            invalidConfig,
            new ValidationContext(invalidConfig),
            invalidResults,
            true
        );
        var valid = Validator.TryValidateObject(
            validConfig,
            new ValidationContext(validConfig),
            validResults,
            true
        );
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(
                invalidResults.Single().ErrorMessage,
                Does.Contain("Either 'TimeoutMs' or 'Amount'")
            );
    ...
    public void IncreasingLoadBalancePolicyConfig_RejectsStartRateGreaterThanMaxRate()
    {
        ...
        Assert.Multiple(() =>
        {
            ...
            Assert.That(
                invalidResults.Single().ErrorMessage,
                Does.Contain("'StartRate' cannot be greater than 'MaxRate'")
            );
            Assert.That(valid, Is.True);
            Assert.That(validResults, Is.Empty);
        });
